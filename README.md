...
M6-0 已冻结三个匿名合成规模档位：100 任务/20 资源/5 区域、500/50/10、2000 个聚合任务/200/20，对应 2/5/15 秒测试目标。M6-1 已实现不读取网络、系统时间或真实数据的确定性场景工厂，三个档位具有固定引用、完整路网、时间窗、资源覆盖和 SHA-256 指纹。当前尚未运行规模规划或生成性能报告，不能声称性能达标；M6 负责形成部署版本，M7 才负责 GitHub 发布和稳定公网地址。
