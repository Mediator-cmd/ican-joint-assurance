...
M6-0 已冻结三个匿名合成规模档位：100 任务/20 资源/5 区域、500/50/10、2000 个聚合任务/200/20，对应 2/5/15 秒测试目标。M6-1 已提供固定场景和 SHA-256 指纹；M6-2 已实现每任务最多 3 个候选资源的有界 CP-SAT，小型/中型必须走 CP-SAT，大型在建模前公开 `model_size_guard` 并返回经过独立复核的确定性安全方案。当前只完成路径与功能验证，尚未生成正式性能报告，不能声称达到时间目标；M6 负责形成部署版本，M7 才负责 GitHub 发布和稳定公网地址。
