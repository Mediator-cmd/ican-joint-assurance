...
M5-1 已提供 `POST /api/v1/assistant/event-drafts`：可把中文航班延误或登机口变更描述解析为绑定场景 version 或运行 revision 的待复核草稿，并返回逐字段依据、缺失字段追问和安全来源标记。当前只使用确定性规则，不调用外部模型；接口不应用事件、不写审计、不修改场景或运行状态，草稿必须在后续人工复核后才能进入现有版本保护链。
...
- 辅助 API 支持确定性中文延误/登机口变更草稿、绝对或相对权威时间、逐字段依据和缺失追问；当前不读取模型密钥，也不自动应用草稿。
...
浏览器访问启动脚本输出的前端 URL。当前后端回归基线为 `165 passed`，前端回归基线为 `19 passed`，并应同时通过 `npm run typecheck` 和 `npm run build`。
