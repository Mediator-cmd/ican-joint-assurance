...
- 阶段：M3 FastAPI 业务接口进行中，M3-0 至 M3-4 已完成。
- 下一单元：M3-5 方案比较、审计查询和 `/api/v1/demo` 兼容输出。
...
- 双击项目根目录的 `启动联保智调.cmd`：刷新演示数据、启动网站并优先使用 Microsoft Edge 打开页面。
...
浏览器访问 `http://127.0.0.1:4173`。当前后端回归基线为 `61 passed`，前端应同时通过 `npm run typecheck` 和 `npm run build`。
